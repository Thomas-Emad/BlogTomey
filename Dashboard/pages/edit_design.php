...
if (isset($_POST['save_change'])) {
  ...
  if ($_FILES['img_site']['error'] == 0) {
    $type_img = explode('/', $_FILES['img_site']['type'])[1];
    if (in_array($type_img, $allow_types_img)) {
      unlink("../../images/design_site/" . $img_site);
      move_uploaded_file($_FILES['img_site']['tmp_name'], "../../images/design_site/img_blog." . $type_img);
      $img_site = "img_blog." . $type_img;
    } else {
      $errors[] = 'type_img';
    }
  }
    ...
    foreach ($errors as $err) {
      if ($err == 'type_icon') {
        ...
        <div class='alert bg-danger text-light d-flex align-items-center' role='alert' >
          ...
          <div style='display: flex; justify-content: space-between; width: 100%;'>
            <span><strong>Can't Upload Change</strong> Only Upload [jpeg, jpg, png, web].</span>
            <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
          </div>
        </div>
        ";
...
<head>
  ...
  <link id="pagestyle" href="../assets/css/edit_design.css" rel="stylesheet" />
</head>
      ...
      <form action="" method="POST" enctype="multipart/form-data">
        <div class="parent">
          ...
          <div class="box">
            ...
            <a href="#icon_site" class="btn btn-secondary m-0" data-bs-toggle="modal">Choose</a>
            <!-- Modal -->
            <div class="modal fade" id="icon_site" tabindex="-1" aria-labelledby="icon_site" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    <h1 class="modal-title fs-5" id="icon_site">Icon Site</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                  </div>
                  <div class="modal-body">
                    <div class="mb-3">
                      <label for="formFile" class="form-label">- Choose Image For Icon Site</label>
                      <input class="form-control" type="file" name="img_site" id="formFile">
                    </div>
                  </div>
                  <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
